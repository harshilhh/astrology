...
export default function PrivacySection() {
  return (
    <section style={{ background:"var(--bg-pale-yellow)", padding:"80px 32px", position:"relative", overflow:"hidden" }}>
      <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse at 50% 50%, rgba(165,32,32,0.04) 0%, transparent 60%)", pointerEvents:"none" }} />
      ...
      <div style={{ maxWidth:900, margin:"0 auto", textAlign:"center", position:"relative", zIndex:1 }}>
        {/* Icon */}
        <div style={{ width:80, height:80, border:"1px solid rgba(165,32,32,0.4)", background:"rgba(165,32,32,0.07)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"2.4rem", margin:"0 auto 28px", position:"relative", boxShadow:"0 0 40px rgba(165,32,32,0.12)" }}>
          🔒
          <div style={{ position:"absolute", inset:-6, border:"1px solid rgba(165,32,32,0.15)" }} />
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:2 }}>
          {[
            ...
            <div key={item.title} style={{ padding:"28px 20px", textAlign:"center", background:"var(--card-bg)", border:"1px solid rgba(165,32,32,0.15)", transition:"all 0.3s ease", position:"relative" }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform="translateY(-4px)"; (e.currentTarget as HTMLElement).style.borderColor="rgba(165,32,32,0.4)"; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform="translateY(0)"; (e.currentTarget as HTMLElement).style.borderColor="rgba(165,32,32,0.15)"; }}
            >
              <div style={{ width:52, height:52, border:"1px solid rgba(165,32,32,0.3)", background:"rgba(165,32,32,0.06)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"1.6rem", margin:"0 auto 14px" }}>
                {item.icon}
              </div>
