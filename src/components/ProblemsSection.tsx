...
export default function ProblemsSection() {
  return (
    <section style={{ background:"var(--bg-cream)", padding:"96px 32px", position:"relative", overflow:"hidden" }}>
      {/* BG glow */}
      <div style={{ position:"absolute", inset:0, background:"radial-gradient(ellipse at 50% 50%, rgba(232,86,42,0.04) 0%, transparent 65%)", pointerEvents:"none" }} />
      <div style={{ position:"absolute", top:0, left:0, right:0, height:1, background:"linear-gradient(90deg, transparent, rgba(232,86,42,0.35), transparent)" }} />
      ...
      <div style={{ maxWidth:1100, margin:"0 auto", position:"relative" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:64 }}>
          ...
          <div style={{ display:"flex", justifyContent:"center", marginBottom:24 }}>
            <svg viewBox="0 0 120 120" fill="none" style={{ width:80, height:80 }}>
              <circle cx="60" cy="60" r="56" stroke="rgba(232,86,42,0.3)" strokeWidth="1"/>
              <circle cx="60" cy="60" r="40" stroke="rgba(232,86,42,0.2)" strokeWidth="0.8" strokeDasharray="3 4"/>
              <polygon points="60,14 104,85 16,85" stroke="rgba(232,86,42,0.5)" strokeWidth="1.2" fill="rgba(232,86,42,0.04)"/>
              <polygon points="60,106 16,35 104,35" stroke="rgba(240,104,48,0.4)" strokeWidth="1" fill="rgba(232,86,42,0.03)"/>
              <circle cx="60" cy="60" r="14" fill="rgba(232,86,42,0.1)" stroke="rgba(232,86,42,0.5)" strokeWidth="1"/>
              <text x="60" y="65" textAnchor="middle" dominantBaseline="central" fontSize="12" fill="#E8562A" fontFamily="serif">ॐ</text>
            </svg>
          </div>
          ...
          {problems.map((p, i) => (
            <a
              ...
              style={{
                ...
                background:"var(--card-bg)", border:"1px solid rgba(232,86,42,0.12)",
                textDecoration:"none", transition:"all 0.25s ease",
              }}
              onMouseEnter={e => {
                (e.currentTarget as HTMLElement).style.background = "rgba(232,86,42,0.06)";
                (e.currentTarget as HTMLElement).style.borderColor = "rgba(232,86,42,0.4)";
                (e.currentTarget as HTMLElement).style.transform = "translateY(-2px)";
              }}
              onMouseLeave={e => {
                (e.currentTarget as HTMLElement).style.background = "var(--card-bg)";
                (e.currentTarget as HTMLElement).style.borderColor = "rgba(232,86,42,0.12)";
                (e.currentTarget as HTMLElement).style.transform = "translateY(0)";
              }}
            >
              <span style={{ width:26, height:26, display:"flex", alignItems:"center", justifyContent:"center", background:"rgba(232,86,42,0.07)", border:"1px solid rgba(232,86,42,0.25)", fontFamily:"var(--font-display)", fontSize:"0.74rem", fontWeight:700, color:"var(--gold-warm)", flexShrink:0 }}>
        ...
        {/* Quote box */}
        <div style={{ textAlign:"center", background:"var(--card-bg)", border:"1px solid rgba(232,86,42,0.2)", padding:"44px 32px", position:"relative" }}>
          <div style={{ position:"absolute", top:0, left:0, width:36, height:36, borderTop:"1px solid rgba(232,86,42,0.5)", borderLeft:"1px solid rgba(232,86,42,0.5)" }} />
          <div style={{ position:"absolute", top:0, right:0, width:36, height:36, borderTop:"1px solid rgba(232,86,42,0.5)", borderRight:"1px solid rgba(232,86,42,0.5)" }} />
          <div style={{ position:"absolute", bottom:0, left:0, width:36, height:36, borderBottom:"1px solid rgba(232,86,42,0.5)", borderLeft:"1px solid rgba(232,86,42,0.5)" }} />
          <div style={{ position:"absolute", bottom:0, right:0, width:36, height:36, borderBottom:"1px solid rgba(232,86,42,0.5)", borderRight:"1px solid rgba(232,86,42,0.5)" }} />
