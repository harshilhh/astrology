...
const serviceCategories = [
  {
    category: "Consultation & Prediction", icon: "📞", accent: "#E8562A",
    services: [
  ...
  {
    category: "Love & Relationships", icon: "♥", accent: "#D9582A",
    services: [
...
export default function ServicesSection() {
  return (
    <section id="services" style={{ background:"var(--bg-white)", padding:"96px 32px", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ position:"absolute", top:0, right:0, width:500, height:500, background:"radial-gradient(circle at 80% 10%, rgba(232,86,42,0.04) 0%, transparent 60%)", pointerEvents:"none" }} />
      ...
      <div style={{ maxWidth:1200, margin:"0 auto" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:72 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:16, marginBottom:16 }}>
            <div style={{ height:1, width:60, background:"linear-gradient(90deg, transparent, rgba(232,86,42,0.4))" }} />
            <span style={{ color:"rgba(232,86,42,0.5)", letterSpacing:"0.4em", fontSize:"0.74rem", fontFamily:"var(--font-ui)" }}>✦</span>
            <div style={{ height:1, width:60, background:"linear-gradient(90deg, rgba(232,86,42,0.4), transparent)" }} />
          </div>
        ...
        {serviceCategories.map((cat) => (
          <div key={cat.category} style={{ marginBottom:64 }}>
            {/* Category header */}
            <div style={{ display:"flex", alignItems:"center", gap:16, marginBottom:24, padding:"16px 20px", background:`rgba(${cat.accent === "#E8562A" ? "46,158,66" : cat.accent === "#D9582A" ? "192,57,43" : cat.accent === "#6A3FA0" ? "106,63,160" : "26,107,58"},0.05)`, border:`1px solid ${cat.accent}22` }}>
              <div style={{ width:44, height:44, background:`${cat.accent}15`, border:`1px solid ${cat.accent}40`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"1.3rem", flexShrink:0 }}>
                {cat.icon}
              </div>
              ...
              {cat.services.map((svc) => (
                <a
                  ...
                  onMouseLeave={e => {
                    ...
                    (e.currentTarget as HTMLElement).style.borderColor="rgba(232,86,42,0.2)";
                  }}
                >
                  {/* Accent dot */}
                  <div style={{ width:8, height:8, background:cat.accent, boxShadow:`0 0 14px ${cat.accent}60`, marginBottom:18 }} />
                  <h4 style={{ fontFamily:"var(--font-display)", fontSize:"0.96rem", fontWeight:700, letterSpacing:"0.08em", color:"var(--text-primary)", marginBottom:10, textTransform:"uppercase" }}>
                    {svc.title}
                  </h4>
                  <p style={{ fontFamily:"var(--font-body)", fontSize:"0.97rem", color:"var(--text-muted)", lineHeight:1.75 }}>
                    {svc.desc}
                  </p>
                  <div style={{ marginTop:20, fontFamily:"var(--font-ui)", fontSize:"0.84rem", fontWeight:700, letterSpacing:"0.25em", textTransform:"uppercase", color:"var(--gold-warm)", display:"flex", alignItems:"center", gap:8 }}>
                    <span>Explore</span>
        ...
        {/* Bottom CTA */}
        <div style={{ textAlign:"center", padding:"52px 32px", background:"var(--bg-cream)", border:"1px solid rgba(232,86,42,0.2)", position:"relative" }}>
          <div style={{ position:"absolute", top:0, left:0, width:32, height:32, borderTop:"1px solid rgba(232,86,42,0.5)", borderLeft:"1px solid rgba(232,86,42,0.5)" }} />
          <div style={{ position:"absolute", top:0, right:0, width:32, height:32, borderTop:"1px solid rgba(232,86,42,0.5)", borderRight:"1px solid rgba(232,86,42,0.5)" }} />
          <div style={{ position:"absolute", bottom:0, left:0, width:32, height:32, borderBottom:"1px solid rgba(232,86,42,0.5)", borderLeft:"1px solid rgba(232,86,42,0.5)" }} />
          <div style={{ position:"absolute", bottom:0, right:0, width:32, height:32, borderBottom:"1px solid rgba(232,86,42,0.5)", borderRight:"1px solid rgba(232,86,42,0.5)" }} />
          ...
          <a href={WA_LINK} target="_blank" rel="noopener noreferrer" className="btn-gold" style={{ padding:"15px 48px", borderRadius:0, fontSize:"1.02rem" }}>
            Consult For Free →
          </a>
        </div>
      </div>
