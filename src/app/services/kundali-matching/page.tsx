          ...
          <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28, marginBottom: 72 }} className="service-detail-grid">
              {[
                ...
                <div key={m.title} style={{ padding: "36px 32px", background: "var(--bg-light-yellow)", borderRadius: 0, border: "1px solid rgba(212,175,55,0.3)", textAlign: "center" }}>
                ...
                {gunas.map((g) => (
                  <div key={g.name} className="mystical-card" style={{ padding: "24px", borderRadius: 0, background: "var(--card-bg)" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                      ...
                      <span style={{ fontFamily: "var(--font-ui)", fontSize: "0.75rem", fontWeight: 700, color: "var(--gold-deep)", background: "rgba(255,215,0,0.15)", borderRadius: 0, padding: "4px 10px", border: "1px solid rgba(212,175,55,0.3)" }}>{g.points} pts</span>
                    </div>
                    <p style={{ fontFamily: "var(--font-body)", fontSize: "0.88rem", color: "var(--text-muted)", lineHeight: 1.65 }}>{g.desc}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 20 }}>
                {scoreGuide.map((s) => (
                  <div key={s.range} style={{ padding: "28px 24px", borderRadius: 0, border: `2px solid ${s.color}40`, background: `${s.color}08`, textAlign: "center" }}>
                    ...
                    <p style={{ fontFamily: "var(--font-body)", fontSize: "0.9rem", color: "var(--text-secondary)" }}>{s.desc}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div style={{ background: "var(--bg-light-yellow)", borderRadius: 0, padding: "36px 28px", border: "1px solid rgba(212,175,55,0.3)" }}>
                ...
                {["Comprehensive compatibility assessment","Remedial measures for a happy marriage","Decision-making support with clear insights","Expert online consultation from Vikram Ji","Manglik dosha identification and remedies","Optimal marriage timing guidance"].map((b) => (
                  <div key={b} style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 12 }}>
                    ...
                    <span style={{ fontFamily: "var(--font-body)", fontSize: "0.9rem", color: "var(--text-secondary)" }}>{b}</span>
                  </div>
                ))}
              </div>
