          ...
          <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            ...
            <div style={{ marginBottom: 72 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 14 }}>
                {symptoms.map((s, i) => (
                  <div key={s} style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "14px 16px", background: i % 2 === 0 ? "var(--bg-light-yellow)" : "white", borderRadius: 0, border: "1px solid rgba(232,86,42,0.2)" }}>
                    <span style={{ width: 26, height: 26, borderRadius: "50%", background: "rgba(232,86,42,0.15)", border: "1px solid rgba(232,86,42,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-heading)", fontSize: "1.02rem", fontWeight: 700, color: "var(--gold-deep)", flexShrink: 0 }}>{i + 1}</span>
                ...
                {lifeEffects.map((e) => (
                  <div key={e.area} className="mystical-card" style={{ padding: "24px", borderRadius: 0, background: "var(--card-bg)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                      <div style={{ width: 44, height: 44, borderRadius: 0, background: "rgba(232,86,42,0.12)", border: "1px solid rgba(232,86,42,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.3rem", flexShrink: 0 }}>{e.icon}</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 28, alignItems: "start" }} className="service-detail-grid">
              <div style={{ background: "var(--bg-light-yellow)", borderRadius: 0, padding: "36px 28px", border: "1px solid rgba(232,86,42,0.3)" }}>
                ...
                {["Expert consultation to identify presence of black magic","Personalized Vedic remedies based on your situation","Powerful mantra chanting and ritual performance","Yantra-based protection and energy cleansing","Post-removal spiritual protection measures","Complete confidentiality throughout the process"].map((s) => (
                  <div key={s} style={{ display: "flex", gap: 10, marginBottom: 12, padding: "10px 14px", background: "var(--card-bg)", borderRadius: 8, border: "1px solid rgba(232,86,42,0.2)" }}>
